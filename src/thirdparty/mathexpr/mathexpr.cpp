...
RVar::RVar(const char*namep,float*pvalp)
...
RFunction::RFunction(float ((*pfuncvalp)(float)))
{
...
{
    type=1;name=new char[1];name[0]=0;
    nvars=1;ppvar=new PRVar[1];ppvar[0]=pvarp;buf=new float[1];
}
...
RFunction& RFunction::operator=(const RFunction& rfunc)
{
    ...
    nvars=rfunc.nvars;
    if(type==1&&nvars){
        ppvar=new PRVar[nvars];buf=new float[nvars];
        int i;for(i=0;i<nvars;i++)ppvar[i]=rfunc.ppvar[i];
    }
    return *this;
}
...
{
    /* Code to use to replace expcitly instead of using a pointer to
...
void IsolateNumbers(char*&s,int nvar,RVar**ppvar,int nfunc,RFunction**ppfunc)//Deletes the old string
{
    ...
    //initialize i2 to fix a wrong compiler warning
    i2=42;
    for(i=0;s[i];i++){
...
/*
float ROperation::Val() const // Won't work if multi-variable functions are included
{
  float v1=ErrVal,v2=ErrVal;
  if(mmb1!=NULL){v1=mmb1->Val();if(fabs(v1)<sqrtminfloat)v1=0;else if(v1==ErrVal||fabs(v1)>sqrtmaxfloat)return ErrVal;};
  if(mmb2!=NULL){v2=mmb2->Val();if(fabs(v2)<sqrtminfloat)v2=0;else if(v2==ErrVal||fabs(v2)>sqrtmaxfloat)return ErrVal;};
  switch(op){
  ...
  case Pow:if(v1==0)return 0;else if((v1>0||!fmod(v2,1))&&v2*log(fabs(v1))<DBL_MAX_EXP)return pow(v1,v2);else return ErrVal;
  case Sqrt:if(v2>=0)return sqrt(v2);else return ErrVal;
  case NthRoot:if(!v1||v2*log(fabs(v1))<DBL_MIN_EXP)return ErrVal;
  else if(v2>=0)return pow(v2,1/v1);else
    if(fmod(v1,2)==1||fmod(v1,2)==-1)return -pow(-v2,1/v1);else return ErrVal;
  case E10:if(v2<DBL_MAX_10_EXP)return v1*pow10(v2);else return ErrVal;
  case Ln:if(v2>0)return log(v2);else return ErrVal;
  case Exp:if(v2<DBL_MAX_EXP)return exp(v2);else return ErrVal;
  case Sin:if(fabs(v2)<inveps)return sin(v2);else return ErrVal;
  case Cos:if(fabs(v2)<inveps)return cos(v2);else return ErrVal;
  case Tg:if(fabs(v2)<inveps)return tan(v2);else return ErrVal;
  case Atan:
    if(mmb2->op==Juxt){v1=mmb2->NthMember(1).Val();v2=mmb2->NthMember(2).Val();return (v1||v2?atan2(v1,v2):ErrVal);}else return atan(v2);
  case Asin:if(v2<-1||v2>1)return ErrVal;else return asin(v2);
  case Acos:if(v2<-1||v2>1)return ErrVal;else return acos(v2);
  case Abs:return fabs(v2);
...
{if(*p==ErrVal||fabs(*p)>sqrtmaxfloat){*(--p)=ErrVal;return;};
    if(*(--p)==ErrVal||fabs(*p)>sqrtmaxfloat){*p=ErrVal;return;};
    *p+=(*(p+1));}
...
{if(fabs(*p)<sqrtminfloat){*--p=0;return;};
    if(*p==ErrVal||fabs(*p)>sqrtmaxfloat){*(--p)=ErrVal;return;};
    if(fabs(*(--p))<sqrtminfloat){*p=0;return;};
    if(*p==ErrVal||fabs(*p)>sqrtmaxfloat){*p=ErrVal;return;};
    *p*=(*(p+1));}
void  Division(float*&p)
{if(fabs(*p)<sqrtminfloat||*p==ErrVal||fabs(*p)>sqrtmaxfloat)
    {*(--p)=ErrVal;return;};
...
{float v2=*p--,v1=*p;
    if(!v1){*p=0;return;};
    if(v2==ErrVal||v1==ErrVal||fabs(v2*log(fabs(v1)))>DBL_MAX_EXP){*p=ErrVal;return;};
    *p=((v1>0||!fmod(v2,1))?pow(v1,v2):ErrVal);}
void  RacineN(float*&p)
{float v2=*p--,v1=*p;
    if(v1==ErrVal||v2==ErrVal||!v1||v2*log(fabs(v1))<DBL_MIN_EXP){*p=ErrVal;return;};
    if(v2>=0){*p=pow(v2,1/v1);return;};
    *p=((fabs(fmod(v1,2))==1)?-pow(-v2,1/v1):ErrVal);}
...
void  RFunc(float*&){}
void  JuxtF(float*&){}
void  Absolu(float*&p){*p=((*p==ErrVal)?ErrVal:fabs(*p));}
void  Oppose(float*&p){*p=((*p==ErrVal)?ErrVal:-*p);}
void  ArcSinus(float*&p)
{*p=((*p==ErrVal||fabs(*p)>1)?ErrVal:asin(*p));}
void  ArcCosinus(float*&p)
{*p=((*p==ErrVal||fabs(*p)>1)?ErrVal:acos(*p));}
...
{*p=((*p==ErrVal||*p>DBL_MAX_EXP)?ErrVal:exp(*p));}
void  Sinus(float*&p)
{*p=((*p==ErrVal||fabs(*p)>inveps)?ErrVal:sin(*p));}
void  Tangente(float*&p)
{*p=((*p==ErrVal||fabs(*p)>inveps)?ErrVal:tan(*p));}
void  Cosinus(float*&p)
{*p=((*p==ErrVal||fabs(*p)>inveps)?ErrVal:cos(*p));}
void  Racine(float*&p)
{*p=((*p==ErrVal||*p>sqrtmaxfloat||*p<0)?ErrVal:sqrt(*p));}
void FonctionError(float*&p){*p=ErrVal;}
inline void ApplyRFunc(PRFunction rf,float*&p)
...
void BCDouble(pfoncld*&pf,pfoncld*pf1,pfoncld*pf2,
              float**&pv,float**pv1,float**pv2,
              float*&pp,float*pp1,float*pp2,
...
void BCSimple(pfoncld*&pf,pfoncld*pf1,float**&pv,float**pv1,
...
void BCFun(pfoncld*&pf,pfoncld*pf1,float**&pv,float**pv1,
