...
//condence fort zone conquered output into one line for multiple wiiners.
static bool sg_condenceConquestOutput = false;
static tSettingItem<bool> sg_condenceConquestOutputConfig("CONDENCE_CONQUEST_OUTPUT", sg_condenceConquestOutput);
...
gSumoZoneHack::gSumoZoneHack( eGrid * grid, const eCoord & pos, bool dynamicCreation )
:gZone( grid, pos, dynamicCreation ), currentState_( State_Unspawned )
{
    if (dynamicCreation){
        SetStateParsing();
    }
